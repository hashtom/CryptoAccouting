...
namespace CryptoAccouting
{
    ...
    {         SfDataGrid sfGrid;         TradeList myTradeList;         //string calendaryear = null;         Exchange thisExchange;         LoadingOverlay loadPop;          public TransactionViewController(IntPtr handle) : base(handle)         {             sfGrid = new SfDataGrid();             //sfGrid.AutoGeneratingColumn += HandleAutoGeneratingColumn;             sfGrid.AllowSorting = true;             sfGrid.AutoGenerateColumns = false;             sfGrid.BackgroundColor = UIColor.FromRGB(236, 184, 138);             //sfGrid.GroupColumnDescriptions.Add(new GroupColumnDescription() { ColumnName = "txid" });              GridDateTimeColumn dateColumn = new GridDateTimeColumn()             {                 MappingName = "TradeDate",                 HeaderText = "Date"             };              GridTextColumn coinColumn = new GridTextColumn()             {
        ...
        }          public override void ViewDidAppear(bool animated)
        {
            ...
            }              if (myTradeList != null)             {                 barbuttonShare.Enabled = myTradeList.Count() > 0 ? true : false;                   var settleCCY = ApplicationCore.GetCrossRate(myTradeList.FiatSettlementCCY);                 var adjrate = 1 / settleCCY.Rate * ApplicationCore.USDCrossRate.Rate;                  labelNumBuy.Text = ApplicationCore.NumberFormat(myTradeList.NumBuy, false, false);                 labelNumSell.Text = ApplicationCore.NumberFormat(myTradeList.NumSell, false, false);
                //labelBTCBuy.Text = ApplicationCore.NumberFormat(myTradeList.);
                labelFiatBuy.Text = ApplicationCore.NumberFormat(myTradeList.TotalValueBuy * adjrate);                 //labelBTCSell.Text = ApplicationCore.NumberFormat(myTradeList.);                 labelFiatSell.Text = ApplicationCore.NumberFormat(myTradeList.TotalValueSell * adjrate);             }         }      } }
