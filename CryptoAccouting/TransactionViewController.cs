...
namespace CryptoAccouting
{
    ...
    {         SfDataGrid sfGrid;         TradeList myTradeList;         //string calendaryear = null;         Exchange thisExchange;         LoadingOverlay loadPop;          public TransactionViewController(IntPtr handle) : base(handle)         {             sfGrid = new SfDataGrid();             //sfGrid.AutoGeneratingColumn += HandleAutoGeneratingColumn;             sfGrid.AllowSorting = true;             sfGrid.AutoGenerateColumns = false;             sfGrid.BackgroundColor = UIColor.FromRGB(236, 184, 138);             //sfGrid.GroupColumnDescriptions.Add(new GroupColumnDescription() { ColumnName = "txid" });              GridDateTimeColumn dateColumn = new GridDateTimeColumn()             {                 MappingName = "TradeDate",                 HeaderText = "Date"             };              GridTextColumn coinColumn = new GridTextColumn()             {
            ...
            GridNumericColumn priceColumn = new GridNumericColumn()
            {
                ...
                HeaderText = "#order",                 NumberDecimalDigits = 0             };              sfGrid.Columns.Add(dateColumn);             sfGrid.Columns.Add(coinColumn);             sfGrid.Columns.Add(settleColumn);             sfGrid.Columns.Add(buysellColumn);             sfGrid.Columns.Add(amountColumn);             sfGrid.Columns.Add(priceColumn);             sfGrid.Columns.Add(valueColumn);             sfGrid.Columns.Add(numtxColumn);          }          public override void ViewDidLoad()
        {
        ...
        }          public override void ViewDidAppear(bool animated)
        {
            ...
            }              if (myTradeList != null)             {                 barbuttonShare.Enabled = myTradeList.Count() > 0 ? true : false;                  //var settleCCY = ApplicationCore.GetCrossRate(myTradeList.SettlementCCY);                 //var adjrate = 1 / settleCCY.Rate * ApplicationCore.USDCrossRate.Rate;                  labelNumBuy.Text = ApplicationCore.NumberFormat(myTradeList.NumBuy, false, false);                 labelNumSell.Text = ApplicationCore.NumberFormat(myTradeList.NumSell, false, false);
                labelBTCBuy.Text = ApplicationCore.NumberFormat(myTradeList.TotalBTCTradeValueBuy, false, true);
                labelOtherBuy.Text = ApplicationCore.NumberFormat(myTradeList.TotalOtherTradeValueBuy, false, true);                 labelBTCSell.Text = ApplicationCore.NumberFormat(myTradeList.TotalBTCTradeValueSell, false, true);                 labelOtherSell.Text = ApplicationCore.NumberFormat(myTradeList.TotalOtherTradeValueSell, false, true);             }         }      } }
