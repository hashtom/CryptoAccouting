...
             if (ApplicationCore.InitializeCore() != EnuAppStatus.Success){                 this.PopUpWarning("some issue!!");             }                                                        myBalance = ApplicationCore.GetMyBalance();              // Configure Segmented control             ConfigureSegmentButton();              // Configure Table source             TableView.RegisterNibForCellReuse(BalanceViewCell.Nib, "BalanceViewCell");             TableView.Source = new BalanceTableSource(myBalance, this);
...
         private void ConfigureSegmentButton()
		...
		public void DeleteItem(Position pos)
		{
			...
			//NavigationController.PopViewController(true);             //NavigationController.PopToRootViewController(true);
		}          public void PopUpWarning(string message){              UIAlertController okAlertController = UIAlertController.Create("Warning",message, UIAlertControllerStyle.Alert);             okAlertController.AddAction(UIAlertAction.Create("OK", UIAlertActionStyle.Default, null));             this.PresentViewController(okAlertController, true, null);         }     } }